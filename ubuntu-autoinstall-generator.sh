...
💁 This script will create fully-automated Ubuntu 22.04.3 LTS Jammy Jellyfish installation media.
...
-e, --use-hwe-kernel    Force the generated ISO to boot using the hardware enablement (HWE) kernel. Not supported
                        by early Ubuntu 22.04.3 release ISOs.
...
-r, --use-release-iso   Use the current release ISO instead of the daily ISO. The file will be used if it already
                        exists.
-s, --source            Source ISO file. By default the latest daily ISO for Ubuntu 22.04.3 will be downloaded
...
#xorriso -indev ubuntu-22.04.3-live-server-amd64.iso -report_el_torito as_mkisofs -V 'Ubuntu 22.04.3 LTS amd64' --modification-date='2022041910231900' --grub2-mbr --interval:local_fs:0s-15s:zero_mbrpt,zero_gpt:'ubuntu-22.04.3-live-server-amd64.iso' --protective-msdos-label -partition_cyl_align off -partition_offset 16 --mbr-force-bootable -append_partition 2 28732ac11ff8d211ba4b00a0c93ec93b --interval:local_fs:7129428d-7137923d::'ubuntu-22.04.3-live-server-amd64.iso' -appended_part_as_gpt -iso_mbr_part_type a2a0d0ebe5b9334487c068b6b72699c7 -c '/boot.catalog' -b '/boot/grub/i386-pc/eltorito.img' -no-emul-boot -boot-load-size 4 -boot-info-table --grub2-boot-info -eltorito-alt-boot -e '--interval:appended_partition_2_start_1782357s_size_8496d:all::' -no-emul-boot -boot-load-size 8496
#xorriso -as mkisofs -r -report_el_torito as_mkisofs -V 'Ubuntu 22.04.3 LTS amd64' --modification-date='2022041910231900' --grub2-mbr --interval:local_fs:0s-15s:zero_mbrpt,zero_gpt:'ubuntu-22.04.3-live-server-amd64.iso' --protective-msdos-label -partition_cyl_align off -partition_offset 16 --mbr-force-bootable -append_partition 2 28732ac11ff8d211ba4b00a0c93ec93b --interval:local_fs:7129428d-7137923d::'ubuntu-22.04.3-live-server-amd64.iso' -appended_part_as_gpt -iso_mbr_part_type a2a0d0ebe5b9334487c068b6b72699c7 -c '/boot.catalog' -b '/boot/grub/i386-pc/eltorito.img' -no-emul-boot -boot-load-size 4 -boot-info-table --grub2-boot-info -eltorito-alt-boot -e '--interval:appended_partition_2_start_1782357s_size_8496d:all::' -no-emul-boot -boot-load-size 8496 -o  "${destination_iso}" .
#xorriso -as mkisofs -r -V 'Ubuntu 22.04.3 LTS MODIF (EFIBIOS)' -o ${destination_iso} -J -joliet-long -b EFI/boot//isolinux.bin -c boot.catalog -boot-load-size 4 -boot-info-table -no-emul-boot -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat 
xorriso -as mkisofs -r -V 'Ubuntu 22.04.3 LTS MODIF (EFIBIOS)' -J -b EFI/boot/bootx64.efi -c boot.catalog -no-emul-boot -boot-load-size 4 -isohybrid-mbr EFI/boot/bootx64.efi -boot-info-table -input-charset utf-8 -eltorito-alt-boot -e EFI/boot/grubx64.efi -no-emul-boot -isohybrid-gpt-basdat -o "${destination_iso}" . &>/dev/null
cd "$OLDPWD" log "👍 Repackaged into ${destination_iso}"
